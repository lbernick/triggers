...
type ConcurrencyControlLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ConcurrencyControl, err error)
...
// List lists all ConcurrencyControls in the indexer.
func (s *concurrencyControlLister) List(selector labels.Selector) (ret []*v1beta1.ConcurrencyControl, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ConcurrencyControl))
	})
	return ret, err
}
...
type ConcurrencyControlNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ConcurrencyControl, error)
...
// List lists all ConcurrencyControls in the indexer for a given namespace.
func (s concurrencyControlNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.ConcurrencyControl, err error) {
...
// Get retrieves the ConcurrencyControl from the indexer for a given namespace and name.
func (s concurrencyControlNamespaceLister) Get(name string) (*v1beta1.ConcurrencyControl, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("concurrencycontrol"), name)
	}
	return obj.(*v1beta1.ConcurrencyControl), nil
}
